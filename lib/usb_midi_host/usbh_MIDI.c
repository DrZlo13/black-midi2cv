...
static USBH_StatusTypeDef USBH_MIDI_InterfaceInit(USBH_HandleTypeDef* phost) {
    ...
    {
        USBH_DbgLog(
            "Cannot Find the interface for MIDI Interface Class.", phost->pActiveClass->Name);
        status = USBH_FAIL;
    } else {
        ...
        if(phost->device.CfgDesc.Itf_Desc[phost->device.current_interface]
               ...
               .bEndpointAddress &
           0x80) {
            MIDI_Handle->InEp = (phost->device.CfgDesc.Itf_Desc[phost->device.current_interface]
                                     .Ep_Desc[0]
                                     .bEndpointAddress);
            MIDI_Handle->InEpSize = phost->device.CfgDesc.Itf_Desc[phost->device.current_interface]
                                        .Ep_Desc[0]
                                        .wMaxPacketSize;
        } else {
            MIDI_Handle->OutEp = (phost->device.CfgDesc.Itf_Desc[phost->device.current_interface]
                                      .Ep_Desc[0]
                                      .bEndpointAddress);
...
static void MIDI_ProcessTransmission(USBH_HandleTypeDef* phost) {
    ...
    case MIDI_SEND_DATA:
        if(MIDI_Handle->TxDataLength > MIDI_Handle->OutEpSize) {
            USBH_BulkSendData(
                phost, MIDI_Handle->pTxData, MIDI_Handle->OutEpSize, MIDI_Handle->OutPipe, 1);
        } else {
            USBH_BulkSendData(
                phost, MIDI_Handle->pTxData, MIDI_Handle->TxDataLength, MIDI_Handle->OutPipe, 1);
        }
...
static void MIDI_ProcessReception(USBH_HandleTypeDef* phost) {
    ...
    case MIDI_RECEIVE_DATA:
        ...
        USBH_BulkReceiveData(
            phost, MIDI_Handle->pRxData, MIDI_Handle->InEpSize, MIDI_Handle->InPipe);
